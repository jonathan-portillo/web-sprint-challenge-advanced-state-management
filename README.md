...
Actions are where you send information into the store, reducers manage how the an applications state changes, and the store is where you hold all that information (the state tree). The store is know as the single source of truth because that all data can be stored withinin one location.

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?

An application state is global and component state is anywhere within the application that holds its own state. Applcation state works when you are dealing with a ton of data and need it to be spread across the application, where as in the component state only wants to handle smaller amounts of data within that specific part of the application.

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux thunk is a middleware that lets you calln upon action creators, which in turn returns a function. It changes the action creators from objects into function which lets us handle async calls better.

5. What is your favorite state management system you've learned and this sprint? Please explain why!

Currently I prefer redux because I have gotten really accustomed to working with it. Having the state encompass the app is really convenient, however I am warming up to context Api.
